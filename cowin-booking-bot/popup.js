...
function getAvailableCenters(dom,slot,timeout, needDate){

    var calender= $(dom).find('.availability-date');
    var colCount=0;
    for(var datecount=0;datecount<7; datecount++){
       if(new Date(calender[datecount].innerText) >= new Date(needDate)){
            colCount=datecount;
            break;
       }
    }
    ...
    if(doseAvailable.length>0){
        ...
        for(var child of doseAvailable){
            if(child.innerText.trim() !='Booked' && child.innerText.trim() !='NA'){
                ...
                if( (count >6 &&  (count%7)>=colCount) || (count <7 && count >=colCount )){
                  ...
                  if(age=='18'){
                      chrome.tabs.executeScript({
                          ...
                          if(results== null){
                              ...
                              getAvailableCenters(results[0],slot,timeout, date);
                          }
                      });
                  }
                  else{
